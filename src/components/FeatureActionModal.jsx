...
export default function FeatureActionModal({ isOpen, onClose, feature, onTryOnSuccess }) {
  ...
  const contentMap = {
    ...
    tryon: {
      icon: <UserSquare2 className="w-8 h-8 text-purple-500" />,
        ...
        <div className="p-8 pb-10">
          {step === 1 ? (
            <div className="animate-fade-in flex flex-col items-center text-center">
              <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 shadow-inner ${data.color}`}>
                {data.icon}
              </div>
              ...
              <button
                ...
                disabled={isLoading}
                className={`w-full py-4 rounded-xl font-semibold text-white transition-all transform active:scale-95 flex justify-center items-center gap-2 ${isLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#080808] hover:bg-black/80 hover:shadow-lg'}`}
              >
                {isLoading ? (
