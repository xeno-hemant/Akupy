        ...
        {feedItems.slice(0, 8).map((product, index) => (
          <div key={index} className="bg-white border border-gray-100 rounded-3xl overflow-hidden group hover:border-primary/30 hover:shadow-xl transition-all duration-300 shadow-sm relative flex flex-col">
            ...
            <div className="p-6 flex flex-col flex-grow">
              <Link to={`/business/${product.businessId}`} className="text-xs font-bold tracking-wider text-gray-400 uppercase mb-2 hover:text-primary transition-colors">
              ...
              <div className="flex items-center justify-between border-t border-gray-100 pt-4 mt-auto">
                <span className="text-xl font-bold text-[#080808]">${Number(product.price).toFixed(2)}</span>
                <button 
                  disabled={!product.inStock}
                  onClick={(e) => {
                    e.preventDefault(); // Prevent link click if nested (it's not, but good practice)
                    addToCart(product, { _id: product.businessId, name: product.businessName });
                  }}
                  className="bg-primary/10 text-primary px-4 py-2 rounded-full text-sm font-semibold hover:bg-primary hover:text-[#080808] transition-colors disabled:opacity-50 disabled:bg-gray-100 disabled:text-gray-400 active:scale-95"
                >
